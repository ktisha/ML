...
\begin{figure}[htbp]
	...
	\end{minipage}%
\end{figure}
\footnotesize\textcolor{gray}{Пример из Python scikit-learn: http://scikit-learn.org/dev}
\end{frame}
...
\begin{frame}\frametitle{Разбор летучки}
Переход в другое пространство с заменой скалярного произведения\\
...
\begin{frame}\frametitle{Разбор летучки}
Покажите, что линейный SVM устойчив к добавлению шумовых признаков. Пусть при обучении
на $n$ признаках результатом работы линейного SVM является разделяющая полоса некоторой
ширины. Покажите, что при добавлении шумового $(n + 1)$-го признака, не способного увеличить
ширину разделяющей полосы, SVM присвоит ему нулевой вес.
\end{frame}

%\begin{frame}\frametitle{Разбор летучки}
%$ \sum\limits_{i=1}^l (1 - M_i(\mathbf{w}, w_0)) + \frac{1}{2C} \sum\limits_{j=1}^n \vert w \vert \rightarrow \min\limits_{\mathbf{w}}$\\
%\vspace{5mm}
%Замена переменных: $u_j =\frac{1}{2} (\vert w_j \vert + w_j)$, $v_j =\frac{1}{2} (\vert w_j \vert + w_j)$\\
%Тогда $w_j = u_j - v_j$ и $\vert w_j \vert = u_j + v_j$\\
%\vspace{5mm}
%$ \sum\limits_{i=1}^l (1 - M_i(u-v, w_0)) + \frac{1}{2C} \sum\limits_{j=1}^n (u_j + v_j) \rightarrow \min\limits_{u, v}$\\
%Чем меньше С, тем больше индексов $j$ таких, что $u_j = v_j = 0$,
%но тогда $w_j = 0$, значит, признак не учитывается
%\end{frame}

\begin{frame}\frametitle{Разбор летучки}
\begin{figure}[htbp]
  \includegraphics[height=200pt, keepaspectratio = true]{images/svm}
\end{figure}
\end{frame}
...
$IGain(p,n) = h(\frac{P}{l}) - \frac{tp+tp}{l}h(\frac{tp}{tp+fp}) - \frac{l-tp-fp}{l}h(\frac{P-tp}{l-tp-fp}) \rightarrow \max$\\
...
Выборка $X^l$\\
\vspace{5mm}
Сгенерируем много датасетов так: будем из $X$ выбирать $l$ точек с замещением, т.е. в новом датасете
